...
if not NotificationsHolder then
    ...
    -- холдер поиска (спрятан)
    local SearchHolder = InsertTheme(Create("Frame", Button, {
        Size = UDim2.fromOffset(0, 22),
        Position = UDim2.new(1, -SelectedFrame.Size.X.Offset - 20, 0.5, 0),
        AnchorPoint = Vector2.new(1, 0.5),
        BackgroundColor3 = Theme["Color Hub 2"],
        BackgroundTransparency = 0.1,
        ClipsDescendants = true
    ...
    local SearchBtn = Create("ImageButton", Button, {
        Size = UDim2.fromOffset(22, 22),
        Position = UDim2.new(1, -SelectedFrame.Size.X.Offset - 25, 0.5, 0),
        AnchorPoint = Vector2.new(1, 0.5),
        BackgroundTransparency = 1,
        AutoButtonColor = false,
    ...
    local function OpenSearch()
        ...
        SearchOpen = true
        CreateTween({SearchHolder, "Size", UDim2.fromOffset(150, 22), 0.25})
    ...
    local function CloseSearch()
        ...
        SearchOpen = false
        CreateTween({SearchHolder, "Size", UDim2.fromOffset(0, 22), 0.25})
        SearchBox.Text = ""
        for _, opt in pairs(_G.__CurrentDropdownOptions or {}) do
    ...
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        for _, opt in pairs(_G.__CurrentDropdownOptions or {}) do
            if opt and opt.nodes and opt.nodes[1] then
                local label = (opt.Name or ""):lower()
                opt.nodes[1].Visible = (q == "" or label:find(q, 1, true))
            end
        end
        CalculateSize()
    end)
    ...
    -- дальше идёт твоя логика DropFrame/ScrollFrame/Options (оставь как было в библиотеке)
    -- просто используй тот же AddOption/RemoveOption/GetOptions, чтобы фильтр работал
end
