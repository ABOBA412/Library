...
if not NotificationsHolder then
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddDropdown(Configs)
            ...
            Image = "rbxassetid://10734950020", -- иконка лупы
           })
           Make("Corner", SearchButton, UDim.new(0, 4))
           Make("Stroke", SearchButton)
local SearchBox = Create("TextBox", SelectedFrame, {
...
SearchButton.MouseButton1Click:Connect(function()
    ...
    if searchOpened then
        ...
        CreateTween({SearchBox, "Size", UDim2.new(0, 100, 1, -4), 0.25, true})
        SearchBox:CaptureFocus()
    else
        CreateTween({SearchBox, "Size", UDim2.new(0, 0, 1, -4), 0.25, true})
...
SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    for _, option in pairs(ScrollFrame:GetChildren()) do
        if option:IsA("TextButton") or option.Name == "Option" then
            option.Visible = option.Name:lower():find(query, 1, true) ~= nil
        end
    end
    CalculateSize()
end)
