...
if not NotificationsHolder then
	...
	-- кнопка/лейбл контейнер (как в original)
	...
	-- Anti click holder (как в оригинале)
	...
	-- === SEARCH UI: располагаем в правом верхнем углу DropFrame (как хотел) ===
	-- SearchHolder расположен у правой границы DropFrame и расширяется влево при открытии
	local SearchHolder = InsertTheme(Create("Frame", DropFrame, {
		Size = UDim2.fromOffset(34, 28),                -- старт: только кнопка
		Position = UDim2.new(1, 8, 0, 6),              -- чуть правее правой границы DropFrame
		AnchorPoint = Vector2.new(0, 0),
		BackgroundTransparency = 1,
		ClipsDescendants = true
	}), "Hub2")
	...
	-- сама кнопка лупы внутри холдера (будет "уезжать" вправо при открытии)
	...
	local function OpenSearch()
		...
		SearchOpen = true
		-- растягиваем holder (вправо от DropFrame), тем самым кнопка визуально "уезжает" вправо и слева становится место для SearchBox
		CreateTween({SearchHolder, "Size", UDim2.fromOffset(SEARCH_W, 28), 0.22})
	...
	-- Опции таблица (доступна для фильтра)
	local Options = {}
	...
	-- OPTIONS logic (почти оригинал, но использует локальную Options таблицу)
	local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected do
		...
		AddOption = function(index, Value)
			...
			ButtonOpt.Activated:Connect(function() Select(Options[Name]) end)
	...
	-- фильтрация: привязываем к SearchBox
	...
	SearchBox.FocusLost:Connect(function()
		-- при потере фокуса закрыть поиск, если пусто
