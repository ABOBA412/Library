...
if not NotificationsHolder then
    ...
    -- вычислим ширину выпадашки (fallback = 152)
    local dropWidth = 152
    pcall(function()
        if SelectedFrame and SelectedFrame.Size and SelectedFrame.Size.X and SelectedFrame.Size.X.Offset then
            dropWidth = SelectedFrame.Size.X.Offset
        end
    end)

    local DropFrame = Create("Frame", NoClickFrame, {
        Size = UDim2.fromOffset(dropWidth or 152, 0),
        BackgroundTransparency = 0.1,
        BackgroundColor3 = Theme["Color Hub 2"],
        AnchorPoint = Vector2.new(0, 1),
        Name = "DropdownFrame",
    ...
    -- SEARCH: правый верхний угол DropFrame
    local SearchHolder = InsertTheme(Create("Frame", DropFrame, {
        Size = UDim2.fromOffset(34, 28),                 -- стартовый размер: только кнопка
        Position = UDim2.new(1, -8, 0, 6),               -- немного влево от правого края внутри DropFrame
    ...
    -- Options logic (здесь используется локальная таблица Options)
    ...
    -- привязка фильтрации к SearchBox
    ...
    -- открытие/закрытие поиска (анимация: holder растягивается слева от правого края)
    SearchBtn.Activated:Connect(function()
        if SearchBox.Visible then
            SearchBox.Text = ""
            SearchBox.Visible = false
            CreateTween({SearchHolder, "Size", UDim2.fromOffset(34, 28), 0.20})
            CreateTween({SearchBtn, "ImageColor3", Theme["Color Text"], 0.18})
            -- восстановим видимость
            for _, opt in pairs(Options) do
                if opt and opt.nodes and opt.nodes[1] then opt.nodes[1].Visible = true end
            end
            CalculateSize()
        else
            CreateTween({SearchHolder, "Size", UDim2.fromOffset(150, 28), 0.22})
