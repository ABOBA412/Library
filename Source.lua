...
if not NotificationsHolder then
			...
			task.delay(0.22, function()
				if SearchBox.Visible then
					...
					if ok and w and w > 0 then
						...
						CreateTween({ SearchBox, "Size", UDim2.fromOffset(newW, 18), 0.18 })
	...
	local function CloseSearch()
		if not SearchOpened then return end
		SearchOpened = false
		CreateTween({ SearchBox, "Size", UDim2.fromOffset(0, 18), 0.18 })
		CreateTween({ SearchButton, "Position", UDim2.new(1, -30, 0.5, 0), 0.18 })
		CreateTween({ SearchButton, "ImageColor3", Color3.fromRGB(255, 255, 255), 0.18 })
		task.delay(0.18, function()
			pcall(function() SearchBox.Visible = false SearchBox.Text = "" end)
			CalculateSize()
		end)
	end

	SearchButton.Activated:Connect(function()
		-- если дроп закрыт — открываем и затем показываем поиск
